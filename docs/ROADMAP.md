...
- Audit correction: many later goals are contract/schema/API complete but still need real data, deeper tests or fuller UI workflows before they should be treated as mature product capability. Manual quick-log is now present in the UI and uses the existing generic ingest/correction contract.
