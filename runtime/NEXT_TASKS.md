...
4. Consume `briefing_snapshots` from the daily-loop epic: a digest endpoint that walks the latest N daily snapshots + a UI "wat is er veranderd" surface that calls `/briefing/:id/diff`. Depends on issue #32 (now done). The current Today tab already renders the daily digest, actions, goal progress, streaks and disclaimer; extend it with briefing history/diff once the digest endpoint exposes latest-N snapshot context.
